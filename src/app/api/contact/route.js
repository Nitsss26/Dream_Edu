...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: 'niteshshr001@gmail.com', // Your Gmail email address
...
export async function POST(request) {
  ...
  console.log(res)
  if (!res || !res.name || !res.email || !res.number || !res.service || !res.message) {
    return NextResponse.status(400).json({ message: "Bad request" });
  }
  try {
    await transporter.sendMail({
      from: res.email, // Sender email address (dynamic)
      to: 'niteshshr001@gmail.com', // Recipient email address
      subject: res.name,
